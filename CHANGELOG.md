...
### New Features
1.  Introduced a Model Context Protocol (MCP) server, mounted at `/mcp`, to expose Beo Echo functionalities for AI clients (Claude, Cursor, VS Code) to manage mocks, routes, logs, replays, actions, config, and workspaces.
2.  Added 60 new tools across 7 areas for MCP, categorized via `_meta.category` and `_meta.categories`, including a `logs_wait` long-poll tool for approximating live log tailing.
3.  Implemented Personal Access Tokens (PATs - `beo_pat_`), complete with model, service, and profile endpoints (`/api/users/me/tokens`), and middleware support.
4.  Integrated an OAuth 2.1 authorization-code flow (PKCE) for OAuth-aware clients, featuring per-request discovery metadata.
5.  Added an `MCPAuthGate` to ensure all `/mcp` requests require a bearer token (PAT or JWT) for authentication.
6.  Included a new MCP section in Settings and a profile dropdown entry, providing endpoint details, client configuration snippets, OAuth steps, and access token management.
7.  Added a `/oauth/consent` route with `returnUrl` handling for login processes.
8.  Updated the landing page and README to include information about MCP.

### Bug Fixes
1.  Resolved an issue where account, instance, and workspace settings were unreachable from the welcome screen when no project was selected.
...
docker pull ghcr.io/yogasw/beo-echo:4.0.0
...
### New Features
1.  Add 'req_body' as a recognized path for parsing request body in JSON logs.
2.  Implement global paste handling for the import modal.
3.  Add support for importing and displaying response body, status, and latency when importing logs.
4.  Add word wrap toggle, find, copy, and JSON path filter to the response body editor.

### Improvements
1.  Enhance JSON log parser to correctly handle headers with array values.
2.  Enhance replay response body tab with formatted code view, language detection, and HTML preview.
...
```bash
# Pull specific version
docker pull ghcr.io/yogasw/beo-echo:3.1.0
