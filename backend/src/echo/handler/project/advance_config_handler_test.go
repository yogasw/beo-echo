...
func TestResetProjectAdvanceConfigHandler(t *testing.T) {
	...
	t.Run("Reset Project Advance Config - Success", func(t *testing.T) {
		// Create test workspace and user
		...
		// Create test project with advance config
		project, err := database.CreateTestProjectWithConfig(workspace.ID, "Test Project 5", generateUniqueAliasAdvance("test-project-advance-5"), string(initialConfigJSON))
		require.NoError(t, err)

		// Setup Gin router
		router := gin.New()
		router.DELETE("/api/projects/:projectId/advance-config", ResetProjectAdvanceConfigHandler)

		// Create HTTP request
		req, err := http.NewRequest("DELETE", "/api/projects/"+project.ID+"/advance-config", nil)
		require.NoError(t, err)
		...
		assert.True(t, response["success"].(bool))
		assert.Equal(t, "Project advance config reset successfully", response["message"])

		// Verify response data
		data := response["data"].(map[string]interface{})
		assert.Equal(t, project.ID, data["project_id"])
		assert.Equal(t, "Test Project 5", data["project_name"])

		// Verify config is actually reset in database
		var updatedProject database.Project
		result := database.GetDB().Where("id = ?", project.ID).First(&updatedProject)
		require.NoError(t, result.Error)
		assert.Empty(t, updatedProject.AdvanceConfig)
	})
}
