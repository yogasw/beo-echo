...
/*
ResetProjectAdvanceConfigHandler resets the advance configuration for a project

Sample curl:

	curl -X DELETE "http://localhost:3600/api/api/projects/my-project/advance-config" \
	  -H "Content-Type: application/json"
*/
func ResetProjectAdvanceConfigHandler(c *gin.Context) {
	handler.EnsureMockService()
	...
	// Reset advance config to empty
	project.AdvanceConfig = ""
	result = database.GetDB().Save(&project)
	if result.Error != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error":   true,
			"message": "Failed to reset project config: " + result.Error.Error(),
		})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "Project advance config reset successfully",
		"data": gin.H{
			"project_id":   project.ID,
			"project_name": project.Name,
		},
