...
func TestHttpExecutor(t *testing.T) {
	...
	t.Run("ExecuteReplay_InvalidURL", func(t *testing.T) {
		...
		// NOTE: http executor automatically adds http:// if protocol is missing, 
		// so "not-a-valid-url" becomes "http://not-a-valid-url" which parses successfully 
		// but fails on network execution.
		req := models.ExecuteReplayRequest{
			Protocol: "http",
			Method:   "GET",
			URL:      "://invalid-url", // This will fail execution.
			Query: map[string]string{
				"param": "value",
			},
		}
