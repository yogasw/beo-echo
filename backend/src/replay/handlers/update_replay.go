...
func UpdateReplayHandler(service *services.ReplayService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		existingReplay, err := service.GetReplay(c.Request.Context(), replayID)
		if err != nil {
			log.Error().
				Err(err).
				Str("project_id", projectID).
		...
		replay, err := service.UpdateReplay(c.Request.Context(), replayID, req)
		if err != nil {
			log.Error().
				Err(err).
				Str("project_id", projectID).
