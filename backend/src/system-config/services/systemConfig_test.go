...
func TestSystemConfigOperations(t *testing.T) {
	...
	t.Run("GetSystemConfig", func(t *testing.T) {
		...
		val, err := GetSystemConfig(BaseURL)
		...
		val, err = GetSystemConfig(FeatureShowPasswordRequirements)
		...
		val, err = GetSystemConfig("TEST_ARRAY:array")
	...
	t.Run("GetConfig with type inference", func(t *testing.T) {
		// Test string config
		strVal, err := GetConfig[string]("BASE_URL")
		...
		// Test boolean config
		boolVal, err := GetConfig[bool]("FEATURE_SHOW_PASSWORD_REQUIREMENTS")
		...
		// Test array config
		arrVal, err := GetConfig[[]string]("TEST_ARRAY")
	...
	t.Run("AddConfig and SetConfigByID", func(t *testing.T) {
		...
		// Add a new config
		config, err := AddConfig(key, value, description, "string")
