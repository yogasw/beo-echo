<script lang="ts">
	...
	import StarlarkAction from './StarlarkAction.svelte';
	...
	// Basic info fields
	let name = action?.name || 'Run Starlark';
	...
	async function handleSubmit() {
		...
		if (!config || !config.script) {
			toast.error('Starlark code is required');
			return;
		}
		...
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-3">
				...
				<div class="flex items-center">
					<div
						class="flex items-center justify-center w-10 h-10 bg-yellow-600/10 dark:bg-yellow-600/10 rounded-lg mr-3 flex-shrink-0"
					>
						<i class="fas fa-python text-yellow-500 text-xl"></i>
					</div>
					<div>
						<h2 class="text-xl font-bold theme-text-primary">
							{action ? 'Edit' : 'Create'} Starlark Action
						</h2>
						<p class="text-sm theme-text-muted">Execute Python-like Starlark code to modify request/response</p>
					</div>
				</div>
			</div>
	...
	<div class="flex-1 overflow-y-auto">
		<form on:submit|preventDefault={handleSubmit} class="h-full">
			<div class="px-6 py-4 space-y-6">
				...
				<div>
					<div class="flex items-center gap-2 mb-3">
						<i class="fas fa-python text-yellow-500 text-sm"></i>
						<h3 class="text-sm font-semibold theme-text-primary">Starlark Code</h3>
					</div>
					<StarlarkAction {config} {executionPoint} on:change={handleConfigChange} />
				</div>
				...
				<div>
					<div class="flex items-center gap-2 mb-3">
						...
						<h3 class="text-sm font-semibold theme-text-primary">
							Filters <span class="text-xs theme-text-muted font-normal">(Optional)</span>
						</h3>
					</div>
