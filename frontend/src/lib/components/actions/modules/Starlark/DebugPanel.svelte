<script lang="ts">
	...
	function runScript() {
		...
		console.log('%c▶ Executing Starlark Action...', 'color: #4ec9b0; font-size: 14px; font-weight: bold;');
		...
		try {
			...
			// Execute script in simulated environment with Starlark-like globals
			const wrappedScript = `
				(function(request, response) {
					// Create simulated Starlark globals
					const json = {
					...
					try {
						// Execute user script with Starlark-like syntax
						${script}
			...
			try {
				// Note: This is a JavaScript simulation for testing purposes
				// The actual Starlark execution happens on the backend
				...
				// If JavaScript eval fails, it might be due to Python syntax
				errorMessage = 'Syntax note: This is a browser simulation. Actual Starlark execution happens on the backend with full Python-like syntax support.';
				console.log('');
				console.log('%c⚠️ Browser simulation limitation', 'color: #ff8c00; font-weight: bold;');
				console.log('The actual Starlark code will run correctly on the backend with full Python-like syntax.');
			}
			...
			console.error(error);
			errorMessage = error.message || 'Script execution failed';
		} finally {
			executing = false;
		}
	}
		...
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
			...
			<div class="flex items-center justify-between p-4 border-b theme-border">
				<div class="flex items-center gap-2">
					...
					<h2 class="text-lg font-semibold theme-text-primary">Test Starlark Action</h2>
				</div>
				<button
					type="button"
