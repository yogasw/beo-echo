<script lang="ts">
	...
	import JavaScriptAction from './JavaScriptAction.svelte';
	...
	// Basic info fields
	let name = action?.name || 'Run JavaScript';
				...
				<div class="flex items-center">
					...
					<div>
						<h2 class="text-xl font-bold theme-text-primary">
							{action ? 'Edit' : 'Create'} JavaScript Action
						</h2>
						<p class="text-sm theme-text-muted">Execute custom JavaScript to modify request/response</p>
					</div>
				</div>
			</div>
			...
			<div class="px-6 py-4 space-y-6">
				...
				<div>
					<div class="flex items-center gap-2 mb-3">
						...
						<h3 class="text-sm font-semibold theme-text-primary">JavaScript Code</h3>
					</div>
					<JavaScriptAction {config} {executionPoint} on:change={handleConfigChange} />
				</div>
