<script lang="ts">
	...
	import { updateResponse } from '$lib/api/BeoApi';
	...
	async function saveChanges() {
		try {
			...
			const updatePromises = filteredRules
				...
				.map((rule) =>
					updateRule(projectId, endpointId, responseId, rule?.id, {
						type: rule.type,
						key: rule.key,
			...
			const createPromises = filteredRules
				.filter((rule) => !rule.id || rule.isNew)
				.map((rule) =>
					createRule(projectId, endpointId, responseId, {
	...
	function toggleLogic(newOperator: string) {
		...
		updateResponse(projectId, endpointId, responseId, {
			rules_logic: newOperator.toLowerCase() as 'and' | 'or'
		}).catch((error) => {
			console.error('Error updating rules logic:', error);
			toast.error('Failed to update rules logic');
		});
	}
...
<div class="w-full rounded-md overflow-hidden shadow-sm" bind:this={containerElement}>
	<div class="p-0">
		...
		<div
			class="bg-gray-100 dark:bg-gray-700 border-b dark:border-gray-600 flex justify-between items-center"
		>
			<div class="flex items-center space-x-4 px-4 py-3">
				...
				<button
					on:click={() => toggleLogic('or')}
					class="logic-button {rulesOperator === 'or'
						? 'text-blue-500 border-blue-500'
						: ThemeUtils.themeTextMuted() +
							' border ' +
							ThemeUtils.themeBorder()} px-2 py-1 rounded text-xs"
				...
				<button
					on:click={() => toggleLogic('and')}
					class="logic-button {rulesOperator === 'and'
						? 'text-blue-500 border-blue-500'
						: ThemeUtils.themeTextMuted() +
							' border ' +
							ThemeUtils.themeBorder()} px-2 py-1 rounded text-xs"
			...
			<div class="flex pr-4 shrink-0">
				<button
					...
					class={isEditing && !hasChanges
						? ThemeUtils.secondaryButton('py-1 px-2 text-xs opacity-60')
						: isEditing
							? ThemeUtils.primaryButton('py-1 px-2 text-xs')
							: ThemeUtils.utilityButton('py-1 px-2 text-xs')}
					type="button"
		...
		{:else if !isEditing}
				...
				>
					<table class="w-full border-collapse table-fixed">
						<thead class="bg-gray-50 dark:bg-gray-750">
							<tr>
								<th
									class="text-left px-4 py-3 text-xs font-semibold {ThemeUtils.themeTextPrimary()} w-1/6"
									>Type</th
								>
								<th
									class="text-left px-4 py-3 text-xs font-semibold {ThemeUtils.themeTextPrimary()}"
									style="flex: 1">Key</th
								>
								<th
									class="text-left px-4 py-3 text-xs font-semibold {ThemeUtils.themeTextPrimary()} w-1/6"
								...
								<th
									class="text-left px-4 py-3 text-xs font-semibold {ThemeUtils.themeTextPrimary()}"
									style="flex: 1">Value</th
								>
								<th
									class="text-left px-4 py-3 text-xs font-semibold {ThemeUtils.themeTextPrimary()} w-16"
									>Actions</th
								>
							</tr>
						</thead>
							...
							{#each localRules as rule, index}
								...
								>
									<td class="px-4 py-2 align-top">
										<span
											class="inline-flex px-2 py-1 text-xs font-medium rounded-full {rule.type ===
											'header'
									...
									<td class="px-4 py-2 align-top">
										...
										{:else}
											<span class="font-medium text-blue-600 dark:text-blue-400 text-xs"
												>{rule.key}</span
											>
										{/if}
									</td>
									...
									<td class="px-4 py-2 align-top">
										<div
											class="break-all whitespace-pre-wrap text-xs {ThemeUtils.themeTextSecondary()}"
										>
											{rule.value}
										</div>
									</td>
			...
			{:else}
				<div class="text-center py-6 rounded-lg border border-dashed {ThemeUtils.themeBorder()}">
					...
					<div class="{ThemeUtils.themeTextMuted()} italic text-xs">
						No rules defined. Response will always match requests.
					</div>
				</div>
			{/if}
				...
				<table class="w-full border-collapse table-fixed">
					...
					<tbody class="divide-y dark:divide-gray-700">
						{#each localRules as rule, i}
							<tr
								class={i % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50/50 dark:bg-gray-750'}
							>
								<td class="px-4 py-2 align-top">
									<select
										value={rule.type}
								...
								<td class="px-4 py-2 align-top">
									<select
										value={rule.operator}
										on:change={(e) =>
											handleOperatorChange(i, (e.target as HTMLSelectElement).value)}
										class="block w-full py-1 px-2 text-xs rounded bg-white dark:bg-gray-700 border {ThemeUtils.themeBorder()} {ThemeUtils.themeTextPrimary()} focus:ring-1 focus:ring-blue-500/50 focus:border-blue-500"
